...
public class Patients {
    ...
    public ResponseEntity<?> updatePatient(HttpServletRequest request, HttpServletResponse response, @PathVariable String id, @RequestBody UpdatePatientDto updatePatient)throws IOException {
        ...
        List<String> permissions = permissionsAttribute != null ? Arrays.asList(permissionsAttribute.toString().split(",")) : Collections.emptyList();

        if(!tokenService.checkPermissions(permissions, Collections.singletonList("300"))){
